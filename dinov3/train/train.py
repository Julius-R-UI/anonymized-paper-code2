...
def do_train(cfg, model, resume=False):
    ...
    with wandb.init(
        ...
        for data in metric_logger.log_every(
            ...
            # Log metrics
            _anchor_grad = loss_dict.get("contrastive/anchor_grad_norm", 0.0)
            if isinstance(_anchor_grad, torch.Tensor):
                try:
                    _anchor_grad = _anchor_grad.item()
                except Exception:
                    _anchor_grad = float(_anchor_grad.detach().cpu().item())
            run.log({
                "lr": lr,
                "wd": wd,
                "mom": mom,
                ...
                "contrastive/valid_anchors": loss_dict["contrastive/valid_anchors"],
                "contrastive/anchor_grad_norm": _anchor_grad,
                "metrics": loss_dict
